...
export function EventCalendarViews({ height, views: userViews, ...restOptions }: EventCalendarViewProps) {
  return (
    <FullCalendar
      height={height}
      /* Abstract Event
      ...
      eventClass={(info) =>
        cn(
          info.isSelected ? ["outline-3", info.isDragging && "shadow-lg"] : "focus-visible:outline-3",
          "outline-ring/50",
        )
      }
      /* Background Event
      ...
      backgroundEventTitleClass={(info) =>
        cn("opacity-50 italic", info.isNarrow || info.isShort ? `p-1 ${xxsTextClass}` : "p-2 text-xs")
      }
      /* List-Item Event
        ----------------------------------------------------------------------------------------- */
      ...
      rowEventClass={(info) =>
        cn("mb-px border-y", info.isStart && "rounded-s-sm border-s", info.isEnd && "rounded-e-sm border-e")
      }
      ...
      columnEventTitleClass={(info) => cn(!info.isShort && (info.isNarrow ? "py-0.5" : "py-1"), "font-medium")}
      /* More-Link
        ----------------------------------------------------------------------------------------- */
      ...
      dayHeaderClass={(info) =>
        cn(
          "justify-center",
          info.inPopover ? "border-b bg-foreground/5" : info.isMajor && "border border-foreground/20",
        )
      }
      dayHeaderInnerClass={(info) =>
        cn(
          ...
          info.inPopover
            ? ["m-1.5 px-1 py-0.5 rounded-sm font-semibold", info.hasNavLink && "hover:bg-foreground/5"]
            : !info.dayNumberText
              ? [
      ...
      dayCellTopClass={(info) => cn(info.isNarrow ? "min-h-0.5" : "min-h-1", "flex flex-row justify-end")}
      ...
      listDayHeaderInnerClass={(info) =>
        cn(
          ...
          !info.level && info.isToday
            ? ["bg-chart-1/50", info.hasNavLink && "hover:bg-chart-1/60 focus-visible:outline-3 outline-ring/50"]
            : info.hasNavLink && "hover:bg-foreground/5",
        )
      }
      ...
      singleMonthClass={(info) =>
        cn(info.multiMonthColumns > 1 && "m-3", info.multiMonthColumns === 1 && !info.isLast && "border-b")
      }
      ...
      singleMonthHeaderInnerClass={(info) =>
        cn("px-1.5 py-0.5 rounded-sm text-base font-semibold", info.hasNavLink && "hover:bg-foreground/5")
      }
      /* Misc Table
        ----------------------------------------------------------------------------------------- */
      ...
      views={{
        ...
        timeGrid: {
          ...
          allDayHeaderInnerClass: (info) => cn("m-2 text-muted-foreground", info.isNarrow ? xxsTextClass : "text-xs"),
          ...
          slotHeaderInnerClass: (info) =>
            cn("relative m-2", info.isNarrow ? `-top-3.5 ${xxsTextClass}` : "-top-4 text-xs", info.isFirst && "hidden"),
